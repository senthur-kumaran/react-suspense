...
function PokemonCacheProvider ({children, cacheTime}) {
  ...
  const getPokemonResource = React.useCallback((name) => {
    ...
    let resource = cache.current[lowecaseName]
    if (!resource) {
      resource = createPokemonResource(lowecaseName)
      cache.current[lowecaseName] = resource
    }
    expirations.current[lowecaseName] = Date.now() + cacheTime
    return resource
  }, [cacheTime])
  return (
